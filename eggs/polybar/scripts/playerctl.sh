...
if [[ -z "$playerctlstatus" ]]; then
  ...
  if [[ -z "$artist" ]]; then
    echo "%{A1:playerctl pause:}$icon_color  $reset_color $song%{A}"
  else
    echo "%{A1:playerctl pause:}$icon_color  $reset_color $artist - $song%{A}"
  fi
else
  echo "%{A1:playerctl play:}$icon_color  $reset_color $artist - $song%{A}"
fi
