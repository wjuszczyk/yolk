...
show = "window_class"
forbidden_classes = "kittyd telegram-desktop TelegramDestop toolkit Polybar Conky Gmrun Pavucontrol".casefold().split(" ")
...
override_names = False #["","","ﱣ"]  # Either a list containing the focused, populated and unfocused versions of workspace name, or False
...
if len(sys.argv) <= 2:
    try:
        ...
        active_text_color = "#98c379" if active_text_color_opt is None else active_text_color_opt
        active_underline = ("#98c379" if underline is not False else None) if active_underline_opt is None else active_underline_opt
        inactive_text_color = "#5a6477" if inactive_text_color_opt is None else inactive_text_color_opt
        inactive_underline = ("#5a6477" if underline is not False else None) if inactive_underline_opt is None else inactive_underline_opt
...
def main():
    if len(sys.argv) <= 2:
        ...
        workspace_order.append(focused_workspace)
        global classcache
        classcache = {}
        try:
            generate(
